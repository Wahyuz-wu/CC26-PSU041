...
class TestInsightEndpointHappyPath:
    def test_insight_returns_200(self, client):
        with patch("main.generate_insight", return_value="## Ringkasan Tren\nPenjualan stabil."):
            with patch("main.build_data_summary", return_value={"historis": {}, "prediksi": {}}):
                resp = client.post("/insight", json={"data": _make_dummy_records(30)})
                assert resp.status_code == 200

    def test_insight_response_has_insight_field(self, client):
        with patch("main.generate_insight", return_value="## Ringkasan Tren\nPenjualan naik."):
            with patch("main.build_data_summary", return_value={"historis": {}, "prediksi": {}}):
                data = client.post("/insight", json={"data": _make_dummy_records(30)}).json()
                assert "insight" in data
                assert isinstance(data["insight"], str)
                assert len(data["insight"]) > 0

    def test_insight_response_has_forecast(self, client):
        with patch("main.generate_insight", return_value="Insight text"):
            with patch("main.build_data_summary", return_value={"historis": {}, "prediksi": {}}):
                data = client.post("/insight", json={"data": _make_dummy_records(30)}).json()
                assert isinstance(data.get("forecast"), list)
                assert len(data["forecast"]) == 7

    def test_insight_response_status_success(self, client):
        with patch("main.generate_insight", return_value="Insight text"):
            with patch("main.build_data_summary", return_value={"historis": {}, "prediksi": {}}):
                data = client.post("/insight", json={"data": _make_dummy_records(30)}).json()
                assert data["status"] == "success"

    def test_insight_response_has_last_date(self, client):
        with patch("main.generate_insight", return_value="Insight text"):
            with patch("main.build_data_summary", return_value={"historis": {}, "prediksi": {}}):
                data = client.post("/insight", json={"data": _make_dummy_records(30)}).json()
                assert "last_date_in_data" in data
...
class TestInsightEndpointValidation:
    def test_insight_less_than_30_rows_returns_422(self, client):
        resp = client.post("/insight", json={"data": _make_dummy_records(29)})
        assert resp.status_code == 422

    def test_insight_empty_data_returns_422(self, client):
        resp = client.post("/insight", json={"data": []})
        assert resp.status_code == 422

    def test_insight_llm_failure_returns_500(self, client):
        with patch("main.build_data_summary", return_value={"historis": {}, "prediksi": {}}):
            with patch("main.generate_insight", side_effect=Exception("LLM timeout")):
                resp = client.post("/insight", json={"data": _make_dummy_records(30)})
                assert resp.status_code == 500

    def test_insight_missing_api_key_returns_422(self, client):
        with patch("main.build_data_summary", return_value={"historis": {}, "prediksi": {}}):
            with patch("main.generate_insight", side_effect=ValueError("OPENROUTER_API_KEY belum dikonfigurasi")):
                resp = client.post("/insight", json={"data": _make_dummy_records(30)})
                assert resp.status_code == 422

    def test_get_insight_not_allowed(self, client):
        assert client.get("/insight").status_code == 405
