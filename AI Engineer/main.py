...
from llm_service import build_data_summary, generate_insight
...
class InsightRequest(BaseModel):
    ...
    def must_have_enough_rows(cls, v):
        if len(v) < 30:
            raise ValueError("Data minimal 30 baris untuk analisis")
        return v
...
@app.post(
    ...
    tags=["Insight"],
    summary="Prediksi + insight narasi berbasis LLM",
)
def insight(request: InsightRequest):
    try:
        ...
        forecast_result = run_inference(df_input)

        summary    = build_data_summary(df_input, forecast_result)
        llm_output = generate_insight(summary)

        logger.info(
            f"Insight generated | last_date={forecast_result['last_date_in_data']} "
            f"| n_rows={len(request.data)}"
        )
